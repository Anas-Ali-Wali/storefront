...
import { TenantInterceptor } from './core/interceptors/jwt.interceptor';
import { TenantResolverService } from './core/services/tenant-resolver.service';
import { ThemeService } from './core/services/theme.service'; // ← naya import

// ← Updated: pehle tenant resolve, phir theme load — sequential (parallel nahi)
export function initTenantAndTheme(
  ...
  return () =>
    tenantResolver
      .resolve()
      .toPromise()
      .then(() => themeService.loadAndApplyTheme());
}
...
@NgModule({
  ...
  providers: [
    ...
    // ← Purana initTenant wala APP_INITIALIZER isse REPLACE ho gaya hai
    {
      provide: APP_INITIALIZER,
      useFactory: initTenantAndTheme,
